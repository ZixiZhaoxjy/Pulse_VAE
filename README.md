...
$$\hat{z}_{\text{mean}} = T_{\text{mean}} \cdot z_{\text{mean}}$$
$$\hat{z}_{\log_\text{var}} = T_{\log_\text{var}} \cdot z_{\log_\text{var}}$$
...
$$\mathbf{z} = \mathbf{z}_{\text{mean}} + e^{\frac{1}{2} \mathbf{z}_{\log_{\text{var}}}} \cdot \boldsymbol{\epsilon}$$
where, $\boldsymbol{\epsilon}$, is a Gaussian noise vector sampled from $\boldsymbol{\epsilon} \sim \mathcal{N}(0, \mathbf{I})$. The exponential term $e^{\frac{1}{2} \mathbf{z}_{\log_{\text{var}}}}$ turns the log variance vector to a positive variance vector. $z$ is the sampled latent variable. 
